...
public class TheAlteningAccount extends Account<TheAlteningAccount> implements TokenAccount {
    ...
    public boolean fetchInfo() {
        ...
        try {
            ...
            MeteorClient.LOG.error("TheAltening凭证无效。");
            return false;
        } catch (Exception e) {
            MeteorClient.LOG.error("获取TheAltening账户信息失败！");
            return false;
        }
    }
    ...
    public boolean login() {
        ...
        try {
            ...
            MeteorClient.LOG.error("登录TheAltening失败。");
            return false;
        }
    }
    ...
    public TheAlteningAccount fromTag(NbtCompound tag) {
        if (tag.getString("name").isEmpty() || tag.getCompound("cache").isEmpty() || tag.getString("令牌").isEmpty()) throw new NbtException();

        name = tag.getString("name").get();
        token = tag.getString("令牌").get();
