...
public class Modules extends System<Modules> {
    ...
    public static void registerCategory(Category category) {
        if (!Categories.REGISTERING) throw new RuntimeException("Modules.registerCategory - 无法在onRegisterCategories回调之外注册类别。");
    ...
    private boolean onBinding(boolean isKey, int value, int modifiers) {
        ...
        if (moduleToBind.keybind.canBindTo(isKey, value, modifiers)) {
            ...
            moduleToBind.info("绑定到（高亮）%s（默认）。", moduleToBind.keybind);
        }
        else if (value == GLFW.GLFW_KEY_ESCAPE) {
            moduleToBind.keybind.set(Keybind.none());
            moduleToBind.info("移除绑定。");
        }
        else return false;
