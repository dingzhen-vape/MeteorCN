...
public abstract class Module implements ISerializable<Module>, Comparable<Module> {
    ...
    public Module(Category category, String name, String description, String... aliases) {
        if (name.contains(" ")) MeteorClient.LOG.warn("模块'{}'的名称中包含无效字符,使其与Meteor Client命令不兼容。", name);
    ...
    public void sendToggledMsg() {
        if (Config.get().chatFeedback.get() && chatFeedback) {
            ...
            ChatUtils.sendMsg(this.hashCode(), Formatting.GRAY, "已切换（高亮）%s（默认） %s（默认）。", title, isActive() ? Formatting.GREEN + "开启" : Formatting.RED + "关闭");
        }
    }
    ...
    public NbtCompound toTag() {
        ...
        tag.put("快捷键", keybind.toTag());
        tag.putBoolean("释放键时切换", toggleOnBindRelease);
        tag.putBoolean("聊天反馈", chatFeedback);
        tag.putBoolean("收藏", favorite);
        tag.put("settings", settings.toTag());
        tag.putBoolean("活跃", active);
    ...
    public Module fromTag(NbtCompound tag) {
        // General
        keybind.fromTag(tag.getCompoundOrEmpty("快捷键"));
        toggleOnBindRelease = tag.getBoolean("释放键时切换", false);
        chatFeedback = !tag.contains("聊天反馈") || tag.getBoolean("聊天反馈", false);
        favorite = tag.getBoolean("收藏", false);

        // Settings
        NbtElement settingsTag = tag.get("settings");
        if (settingsTag instanceof NbtCompound) settings.fromTag((NbtCompound) settingsTag);

        boolean active = tag.getBoolean("活跃", false);
