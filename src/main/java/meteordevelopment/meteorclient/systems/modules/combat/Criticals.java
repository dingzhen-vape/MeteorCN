...
public class Criticals extends Module {
    ...
    private final SettingGroup sgMace = settings.createGroup("重锤");

    private final Setting<Mode> mode = sgGeneral.add(new EnumSetting.Builder<Mode>()
        .name("模式")
        .description("关于Criticals如何运作的模式。")
    ...
    private final Setting<Double> extraHeight = sgMace.add(new DoubleSetting.Builder()
    	.name("额外高度")
    	.description("要伪装的额外高度。更多高度意味着更多伤害。")
    	.defaultValue(0.0)
        .min(0)
        .sliderRange(0, 100)
        .visible(mace::get)
    ...
    public Criticals() {
        super(Categories.Combat, "刀爆", "当你击中目标时执行暴击攻击。");
    }
