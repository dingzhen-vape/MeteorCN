...
public class WaypointsModule extends Module {
    ...
    private final Setting<Boolean> dpChat = sgDeathPosition.add(new BoolSetting.Builder()
        .name("聊天栏")
        .description("在你死亡时发送一个带有你位置的聊天消息")
        .defaultValue(false)
        .build()
    );

    public WaypointsModule() {
        super(Categories.Render, "创建路标", "允许你创建路标.");
    }
    ...
    private void onRender2D(Render2DEvent event) {
        ...
        for (Waypoint waypoint : Waypoints.get()) {
            ...
            if (distToCenter <= textRenderDist) {
                ...
                // Render distance
                String distText = String.format("%d 格子", (int) Math.round(dist));
    ...
    public void addDeath(Vec3d deathPos) {
        ...
        if (dpChat.get()) {
            MutableText text = Text.literal("死在 ");
    ...
    private void cleanDeathWPs(int max) {
        ...
        for (Iterator<Waypoint> it = Waypoints.get().iterator(); it.hasNext();) {
            ...
            if (wp.name.get().startsWith("死亡 ") && wp.icon.get().equals("skull")) {
                oldWpC++;
