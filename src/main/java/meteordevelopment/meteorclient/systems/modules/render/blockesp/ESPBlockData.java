...
public class ESPBlockData implements ICopyable<ESPBlockData>, ISerializable<ESPBlockData>, IChangeable, IBlockData<ESPBlockData>, IScreenFactory {
    ...
    public ESPBlockData fromTag(NbtCompound tag) {
        ...
        lineColor.fromTag(tag.getCompoundOrEmpty("线条颜色"));
        sideColor.fromTag(tag.getCompoundOrEmpty("侧面颜色"));

        tracer = tag.getBoolean("追踪线", false);
        tracerColor.fromTag(tag.getCompoundOrEmpty("追踪线颜色"));

        changed = tag.getBoolean("改变", false);
