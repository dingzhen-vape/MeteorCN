...
public class HighwayBuilder extends Module {
    ...
    private final Setting<Boolean> disconnectOnToggle = sgGeneral.add(new BoolSetting.Builder()
        .name("切换时断开连接")
        .description("当模块关闭时自动断开连接,例如当没有足够的方块时。")
        .defaultValue(false)
        .build()
    );
    ...
    private final Setting<Boolean> destroyCrystalTraps = sgGeneral.add(new BoolSetting.Builder()
        .name("摧毁水晶陷阱")
        .description("使用弓箭从远处安全解除水晶陷阱,推荐使用无限弓。")
        .defaultValue(true)
        .build()
    );
    ...
    private final Setting<Boolean> doubleMine = sgDigging.add(new BoolSetting.Builder()
        .name("双重挖矿")
        .description("是否在适用时进行双重挖矿（同时进行普通挖矿和数据包挖矿）。")
        .defaultValue(true)
        .build()
    );
    ...
    private final Setting<Boolean> fastBreak = sgDigging.add(new BoolSetting.Builder()
        .name("快速破坏")
        .description("在双重挖矿时,是否比正常情况下更快地完成破坏方块。")
        .defaultValue(true)
    ...
    private final Setting<Integer> savePickaxes = sgDigging.add(new IntSetting.Builder()
        .name("保存镐子")
        .description("确保保存多少把镐子。达到这个数量时,库存会触发补充或模块关闭。")
        .defaultValue(1)
        .range(0, 36)
        .sliderRange(0, 36)
        .visible(() -> !dontBreakTools.get())
    ...
    private final Setting<Integer> blocksPerTick = sgDigging.add(new IntSetting.Builder()
        .name("每刻放置")
        .description("每个刻中最多可以挖掘的方块数量。仅适用于可以立即破坏的方块。")
        .defaultValue(1)
        .range(1, 100)
        .sliderRange(1, 25)
        .build()
    ...
    private final Setting<Boolean> ejectUselessShulkers = sgInventory.add(new BoolSetting.Builder()
        .name("弹出无用的箱子")
        .description("是否弹出无用的末影箱。警告 - 会丢弃任何不包含可放置方块、镐子或食物的末影箱。请小心使用你的工具包。")
        .defaultValue(true)
        .build()
    );
    ...
    private final Setting<Boolean> searchEnderChest = sgInventory.add(new BoolSetting.Builder()
        .name("搜索末影箱")
        .description("搜索你的末影箱以找到可用的物品。使用时小心,尤其是让它搜索末影箱时。")
        .defaultValue(false)
        .build()
    );
    ...
    private final Setting<BlockadeType> blockadeType = sgInventory.add(new EnumSetting.Builder<BlockadeType>()
        .name("末影箱封锁类型")
        .description("使用哪种封锁类型（挖掘末影箱时放置的结构）。")
    ...
    public final Setting<Integer> saveEchests = sgInventory.add(new IntSetting.Builder()
        .name("保存末影箱")
        .description("确保保存多少个末影箱。达到这个数量时,库存会触发补充或模块关闭。")
        .defaultValue(2)
        .range(0, 64)
        .sliderRange(0, 64)
        .visible(mineEnderChests::get)
    ...
    public void onActivate() {
        ...
        if (blocksPerTick.get() > 1 && rotation.get().mine) warning("启用旋转后,每个刻最多只能破坏一个方块。");
        if (placementsPerTick.get() > 1 && rotation.get().place) warning("启用旋转后,每个刻最多只能放置一个方块。");

        if (Modules.get().get(InstantRebreak.class).isActive()) warning("建议禁用即时重破模块,改用 'instantly-rebreak-echests' 设置,以避免错误。");
    }
    ...
    public void onDeactivate() {
        ...
        if (displayInfo) {
            ...
            info("破坏方块: (突出显示)%d", blocksBroken);
            info("放置方块: (突出显示)%d", blocksPlaced);
        }
    }
    ...
    public MutableText getStatsText() {
        ...
        text.append(String.format("%s破坏方块: %s%d\n", Formatting.GRAY, Formatting.WHITE, blocksBroken));
        text.append(String.format("%s放置方块: %s%d", Formatting.GRAY, Formatting.WHITE, blocksPlaced));
        ...
        Restock {
            ...
            protected void start(HighwayBuilder b) {
                ...
                if (slot == -1) {
                    ...
                    } else b.error("无法为 '" + b.restockTask.item() + "'.");
        ...
        DefuseCrystalTraps {
            ...
            protected void start(HighwayBuilder b) {
                if (!InvUtils.find(Items.BOW).found() || (!InvUtils.find(itemStack -> itemStack.getItem() instanceof ArrowItem).found() && !b.mc.player.getAbilities().creativeMode)) {
                    ...
                    b.warning("没有找到弓来摧毁水晶陷阱。正在关闭此设置。");
                    b.setState(Forward);
                }
            ...
            protected void tick(HighwayBuilder b) {
                ...
                if (shots >= 3) {
                    ...
                    b.warning("检测到水晶可能卡住。已将其添加到忽略列表并继续前进。");
                    b.setState(Forward);
                    b.mc.interactionManager.stopUsingItem(b.mc.player);
                    b.drawingBow = false;
                    return;
            ...
            if (bestStack.isIn(ItemTags.PICKAXES)) {
                ...
                if (count <= b.savePickaxes.get() && !(b.restockTask.pickaxes && bestStack.getMaxDamage() - bestStack.getDamage() > (bestStack.getMaxDamage() * (b.breakDurability.get() / 100)))) {
                    if (!b.restockTask.pickaxes && (b.searchEnderChest.get() || b.searchShulkers.get())) {
                        ...
                        b.error("找到的镐子少于所需的最低数量：" + count + "/" + (b.savePickaxes.get() + 1));
                    }
            ...
            return new MBPIterator() {
                ...
                private MBlockPos get(int i) {
                    ...
                    return switch (i) {
                        ...
                        default -> throw new IllegalStateException("意外的值：" + i);
                    };
                }
    ...
    private class RestockTask {
        ...
        public String item() {
            if (materials) return "建筑材料";
            if (pickaxes) return "镐子";
            if (food) return "食物";
            return "未知";
        }
    }
