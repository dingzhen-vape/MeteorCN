...
public class Waypoints extends System<Waypoints> implements Iterable<Waypoint> {
    public static final String[] BUILTIN_ICONS = {"square", "circle", "triangle", "star", "diamond", "头骨"};
    ...
    @Override
    public void init() {
        File iconsFolder = new File(new File(MeteorClient.FOLDER, "创建路标"), "icons");
        iconsFolder.mkdirs();
    ...
    public File getFile() {
        ...
        return new File(new File(MeteorClient.FOLDER, "创建路标"), Utils.getFileWorldName() + ".nbt");
    }
    ...
    public NbtCompound toTag() {
        ...
        tag.put("创建路标", NbtUtils.listToTag(waypoints));
        return tag;
    }
    ...
    public Waypoints fromTag(NbtCompound tag) {
        ...
        for (NbtElement waypointTag : tag.getListOrEmpty("创建路标")) {
